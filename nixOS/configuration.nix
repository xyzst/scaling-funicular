{ config, pkgs, ... }: {
  ...
  systemd = {
    services = {
      timeMachineUserSetup = {
        description = "Idempotent directory setup for timemachine user";
        requiredBy = [
          "netatalk.service"
        ];
        script = '' mkdir -p /tmbak
                    chown timemachine:users /tmbak
                    chmod 0750 /tmbak '';
      };
    };
  };
  ...
  users = {
    ...
    users = {
      ...
      timemachine = {
        initialHashedPassword = "$6$4CanI/KMP$j6jb8n1njLqqVPzsi5OgJgKa/L62UHSUr/o4.Q6qEXrJq7N3fFyi36kOAn5kG6cnlyvv.bsG7Kug/gaV8j2Lx.";
        description = "Time machine use only";
      };
    };
    motd = 
      ''
